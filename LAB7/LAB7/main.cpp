...
CRITICAL_SECTION criticalSection, criticalSection1;
HANDLE empty1;
HANDLE full;
...
DWORD WINAPI producer(LPVOID v) {
	...
	for (int i = 0; i < parametrs->number; i++) {
		...
		cout << "Thread: " << parametrs->threadNumber << " create number: " << num << endl;
...
DWORD WINAPI consumer(LPVOID v) {
	...
	for (int i = 0; i < parametrs->number; i++) {
		...
		cout << "Thread: " << parametrs->threadNumber << " used element: " << num << endl;
...
int main() {
	...
	cout << "Input number of consumer threads: ";
